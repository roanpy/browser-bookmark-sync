...
Security fixes target the latest `main` branch and the latest published release, currently `v0.2.1`. Reproduce older-release issues against both the affected release and current `main` before reporting them.

安全修复面向最新 `main` 分支和最新公开版本（当前为 `v0.2.1`）。旧版本问题请同时在受影响版本和当前 `main` 上确认后再报告。
